...
const state = {
  ...
  ivuRate: 0.115           // 11.5% por defecto, se actualiza desde configuración
};
...
async function cargarConfiguracion() {
  try {
    ...
    if (typeof data.ivuRate === "number") {
      state.ivuRate = data.ivuRate / 100; // guardas 11.5 y lo paso a 0.115
    }
    ...
    if (form) {
      ...
      if (data.ivuRate != null) form.ivuRate.value = data.ivuRate;
    }
...
async function guardarConfiguracion(e) {
  ...
  const ivuNumber = parseFloat(form.ivuRate.value || "11.5");
  const payload = {
    ...
    ivuRate:         isNaN(ivuNumber) ? 11.5 : ivuNumber,
  ...
  try {
    ...
    state.ivuRate = payload.ivuRate / 100;
...
function abrirEntrega(ingresoId) {
  openDialog({
    ...
    onSubmit: async (values) => {
      ...
      const ivu = +(subtotal * state.ivuRate).toFixed(2);
      const totalConIvu = +(subtotal + ivu).toFixed(2);
...
function nuevaCotizacion() {
  openDialog({
    ...
    onSubmit: async (values) => {
      const subtotal = parseFloat(values.total || "0") || 0;
      const ivu = +(subtotal * state.ivuRate).toFixed(2);
...
async function cargarPrecios() {
  ...
  try {
    // SOLO un orderBy para evitar índice compuesto
    ...
    // Pasar a array y ordenar por servicio en JavaScript
    const items = [];
    snap.forEach(doc => {
      const d = doc.data();
      items.push({
