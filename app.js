// app.js — Nexus Auto Pro 2026 (v3, con modales internos)
...
/**
 * Abre modal con campos dinámicos.
...
/* Cerrar al hacer click en cancelar o fuera */
if (dialogCancelBtn) {
...
async function cargarIngresosHoy() {
  ...
  try {
    ...
    snap.forEach(doc => {
      const d = doc.data();
      const fechaDate = d.createdAt?.toDate?.() || null;
      const fecha = fechaDate
        ? fechaDate.toLocaleDateString("es-PR", { year: "2-digit", month: "2-digit", day: "2-digit" })
        : "";
      total++;
      if (d.tipoTrabajo === "mecanica") mecanica++;
      if (!ultimaFecha || (fechaDate && fechaDate > ultimaFecha)) ultimaFecha = fechaDate;
...
function nuevoPrecio() {
  openDialog({
    ...
    fields: [
      ...
      { name: "precio", label: "Precio base (sin $)", type: "number", required: true }
    ],
    onSubmit: async (values) => {
