...
const state = {
  ...
  config: {
    ...
    logoUrl: "",
    driveUrl: "",
    ivuRate: 11.5 // porcentaje (%)
  },
  preciosCache: [] // para usar lista de precios en cotizaciones
};
...
function getIvuRateDecimal() {
  const r = parseFloat(state.config.ivuRate);
  if (isNaN(r) || r <= 0) return 0.115; // 11.5% por defecto
  return r / 100;
}
...
async function cargarConfiguracion() {
  try {
    ...
    state.config = {
      ...
      tallerTelefono:  data.tallerTelefono  || "",
      tallerEmail:     data.tallerEmail     || "",
      tallerDireccion: data.tallerDireccion || "",
      logoUrl:         data.logoUrl         || "",
      driveUrl:        data.driveUrl        || "",
      ivuRate:         data.ivuRate != null ? Number(data.ivuRate) : 11.5
    };

    const form = qs("#configForm");
    if (form) {
      form.tallerNombre.value    = state.config.tallerNombre;
      form.tallerTelefono.value  = state.config.tallerTelefono;
      form.tallerEmail.value     = state.config.tallerEmail;
      form.tallerDireccion.value = state.config.tallerDireccion;
      form.logoUrl.value         = state.config.logoUrl;
      form.driveUrl.value        = state.config.driveUrl;
      form.ivuRate.value         = state.config.ivuRate;
    }
...
async function guardarConfiguracion(e) {
  ...
  const ivuRate  = form.ivuRate.value ? Number(form.ivuRate.value) : 11.5;
...
// Versión sin where+orderBy (para evitar índices)
...
function abrirEntrega(ingresoId) {
  openDialog({
    ...
    onSubmit: async (values) => {
      ...
      const ivuRate = getIvuRateDecimal();
      ...
      const facturaData = {
        ...
        subtotal: total,
        ivu: +(total * ivuRate).toFixed(2),
        total: +(total * (1 + ivuRate)).toFixed(2),
...
// también sin where+orderBy combinado
...
async function generarFacturaPDF(facturaId) {
  try {
    ...
    const taller = state.config.tallerNombre || "Nexus Auto Pro 2026";
    const tel    = state.config.tallerTelefono || "";
    const dir    = state.config.tallerDireccion || "";

    pdf.setFontSize(16);
    pdf.text(taller, 14, 16);
    pdf.setFontSize(10);
    if (dir) pdf.text(dir, 14, 22);
    if (tel) pdf.text("Tel: " + tel, 14, 28);

    pdf.setFontSize(14);
    pdf.text("FACTURA", 160, 16);

    pdf.setFontSize(10);
    pdf.text(`Factura: ${d.numero || facturaId}`, 160, 24);
    const fechaTxt = d.fecha?.toDate?.().toLocaleDateString("es-PR") || new Date().toLocaleDateString("es-PR");
    pdf.text(`Fecha: ${fechaTxt}`, 160, 30);

    pdf.setFontSize(11);
    pdf.text("Datos del cliente:", 14, 40);
    pdf.setFontSize(10);
    ...
    pdf.text(`Vehículo: ${d.vehiculoMarca || ""} ${d.vehiculoModelo || ""}`, 14, 74);
    if (d.vehiculoVin) pdf.text(`VIN: ${d.vehiculoVin}`, 14, 80);

    pdf.text("Detalle de trabajos:", 14, 92);
    ...
    y += 6;
    pdf.text(`IVU: $${(d.ivu || 0).toFixed(2)}`, 140, y);
    y += 6;
    pdf.setFontSize(12);
    pdf.text(`TOTAL: $${(d.total || 0).toFixed(2)}`, 140, y);
...
/* --- soporte: cargar lista de precios en memoria para cotizaciones --- */
async function cargarPreciosEnCache() {
  if (state.preciosCache.length) return;
  try {
    ...
    console.error("Error cargando precios cache:", err);
  }
}

function nuevaCotizacion() {
  (async () => {
    await cargarPreciosEnCache();

    const opciones = state.preciosCache.map(p => ({
      value: p.id,
      label: `${p.categoria} · ${p.servicio} ($${p.precio.toFixed(2)} ${p.tipo === "hora" ? "/h" : ""})`
    }));
    ...
    openDialog({
      ...
      fields: [
        ...
        { name: "vehiculoTablilla",label: "Tablilla del vehículo", type: "text", required: false },
        {
          ...
          options: opciones.length
            ? opciones
            : [{ value: "", label: "No hay servicios en la lista de precios" }]
        },
        {
          name: "cantidad",
          label: "Cantidad (horas / unidades)",
          type: "number",
          required: true,
        ...
        {
          name: "montoExtra",
          label: "Otros cargos (opcional, sin $)",
          type: "number",
          required: false,
      ...
      onSubmit: async (values) => {
        const precioId   = values.precioId;
        const cantidad   = parseFloat(values.cantidad || "1") || 1;
        const montoExtra = parseFloat(values.montoExtra || "0") || 0;

        const item = state.preciosCache.find(p => p.id === precioId);
        const precioBase = item ? item.precio : 0;

        const subtotalBase = precioBase * cantidad + montoExtra;
        const ivuRate = getIvuRateDecimal();
        ...
        const data = {
          ...
          descripcion:     values.descripcion     || (item ? item.servicio : ""),
          precioId:        precioId || null,
          cantidad,
          montoExtra,
          subtotal: subtotalBase,
          ivu: +(subtotalBase * ivuRate).toFixed(2),
          total: +(subtotalBase * (1 + ivuRate)).toFixed(2),
          creadoPor: state.user?.email || "PIN_LOCAL",
          createdAt: firebase.firestore.FieldValue.serverTimestamp()
        };
...
async function generarCotizacionPDF(cotizacionId) {
  try {
    ...
    pdf.setFontSize(14);
    pdf.text("COTIZACIÓN", 160, 16);

    pdf.setFontSize(10);
    pdf.text(`Cotización: ${d.numero || cotizacionId}`, 160, 24);
    const fechaTxt = d.fecha?.toDate?.().toLocaleDateString("es-PR") || new Date().toLocaleDateString("es-PR");
    pdf.text(`Fecha: ${fechaTxt}`, 160, 30);
    ...
    y += 6;
    pdf.setFontSize(12);
    pdf.text(`TOTAL COTIZACIÓN: $${(d.total || 0).toFixed(2)}`, 140, y);
  ...
  try {
    ...
    snap.forEach(doc => {
      ...
      html += `<tr>
        <td>${d.categoria || ""}</td>
        <td>${d.servicio  || ""}</td>
        <td>${tipo === "hora" ? "Servicio por hora" : "Producto / servicio fijo"}</td>
  ...
  openDialog({
    ...
    fields: [
      ...
      },
      {
        ...
        label: "Servicio",
        type: "text",
        placeholder: "Cambio de aceite, Pulido completo...",
        required: true
      },
      {
        name: "tipo",
