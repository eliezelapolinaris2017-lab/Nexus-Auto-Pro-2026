...
async function cargarIngresos() {
  ...
  try {
    // Traemos TODOS los ingresos y filtramos en el cliente
    ...
    // Sólo abiertos
    const abiertos = docs.filter(d => (d.data.estado || "abierto") === "abierto");

    // Orden por fecha creación desc
    abiertos.sort((a, b) => {
    ...
    abiertos.forEach(reg => {
      ...
      html += `<tr>
        ...
        <td>${d.tipoTrabajo || "-"}</td>
        <td>
          <button class="btn small" data-entregar="${reg.id}">Registrar entrega</button>
        </td>
      </tr>`;
    });
...
async function cargarEntregas() {
  ...
  try {
    ...
    // Orden por fecha de entrega desc
    entregas.sort((a, b) => {
      const ta = a.entregaAt?.toMillis?.() || 0;
      const tb = b.entregaAt?.toMillis?.() || 0;
      return tb - ta;
    });
...
async function cargarFacturas() {
  ...
  try {
    ...
    // Filtramos por estado si aplica
    const filtradas = docs.filter(reg => {
      const est = reg.data.estado || "";
      if (filtro === "pendiente") return est === "pendiente";
      if (filtro === "pagada")    return est === "pagada";
      return true;
    });

    // Orden por fecha desc
    filtradas.sort((a, b) => {
      const ta = a.data.fecha?.toMillis?.() || 0;
      const tb = b.data.fecha?.toMillis?.() || 0;
      return tb - ta;
    });

    if (!filtradas.length) {
      cont.innerHTML = "<p class='placeholder'>No hay facturas para ese filtro.</p>";
      return;
    }
    ...
    filtradas.forEach(reg => {
      ...
      html += `<tr>
        ...
        <td>
          <button class="btn small" data-marcar-pagada="${reg.id}">Marcar pagada</button>
          <button class="btn small" data-imprimir-factura="${reg.id}">Imprimir</button>
        </td>
      </tr>`;
    });
...
async function cargarPrecios() {
  ...
  try {
    ...
    // Ordenamos por categoría y luego servicio
    items.sort((a, b) => {
